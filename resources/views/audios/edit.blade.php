...
@section('content')
    <div class="w-full rounded-xl bg-white p-6 shadow-sm">
        <div class="mb-4 text-sm text-gray-500">
            Audio ID: <span class="font-mono text-gray-700">{{ $audio->audio_id }}</span>
        </div>

        <form method="POST" action="{{ route('audios.update', $audio) }}" enctype="multipart/form-data" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="category_id" name="category_id" required
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category_id', $audio->category_id) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input id="title" type="text" name="title" value="{{ old('title', $audio->title) }}" placeholder="e.g. Tum Hi Ho" required
                       class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-gray-500 focus:outline-none focus:ring-1 focus:ring-gray-500">
            </div>

            <div>
                <label for="artist" class="block text-sm font-medium text-gray-700">Artist</label>
                <input id="artist" type="text" name="artist" value="{{ old('artist', $audio->artist) }}" placeholder="e.g. Arijit Singh" required
                       class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-gray-500 focus:outline-none focus:ring-1 focus:ring-gray-500">
            </div>

            <div>
                <label for="year_of_publish" class="block text-sm font-medium text-gray-700">Year of Publish</label>
                <input id="year_of_publish" type="number" name="year_of_publish" value="{{ old('year_of_publish', $audio->year_of_publish) }}"
            ...
            <label class="flex items-center gap-2 text-sm text-gray-700">
                <input type="checkbox" name="status" value="1" {{ old('status', $audio->status) ? 'checked' : '' }} class="rounded border-gray-300">
                Active
            </label>
